...
test("Deve criar um pedido com 3 itens com cupom de desconto", function () {
    ...
    order.addItem(new Item(2, "Instrumentos Musicais", "Amplificador", 5000), 1);
    order.addItem(new Item(3, "Instrumentos Musicais", "Cabo", 30), 3);
    order.addCoupon(new Coupon("VALE20", 20, new Date('2022-10-01')));
    const total = order.getTotal();
...
test("Deve calcular o valor o frete de um pedido", function () {
    const order = new Order("847.903.332-05");
    order.addItem(new Item(1, "Instrumentos Musicais", "Guitarra", 1000, 100, 30, 10, 3), 1);
    order.addItem(new Item(2, "Instrumentos Musicais", "Amplificador", 5000, 50, 50, 50, 6), 1);
    const shippingTotal = order.getShippingTotal();
    expect(shippingTotal).toBe(90);
});